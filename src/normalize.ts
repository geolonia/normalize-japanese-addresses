 ...
 * 町丁目データを取得します。
 *
 * @param url - 相対パスを作成する文字列。".../ja" のパスが自動的に冒頭に付加されます。
 *
 * @returns Fetch レスポンス
 *
 * @see https://github.com/geolonia/japanese-addresses/tree/develop/api/ja
 */
...
const normalizeTownName = async (
  ...
  const townPatterns = await getTownRegexPatterns(pref, city, { fetch })
...
export const createNormalizer: (fetch: DataFetcher) => Normalizer =
  (fetch) =>
  async (address, option = defaultOption) => {
    /**
    ...
    const prefectures = await getPrefectures({ fetch })
    ...
    if (city && option.level >= 3) {
      const normalized = await normalizeTownName(addr, pref, city, { fetch })
      if (normalized) {
