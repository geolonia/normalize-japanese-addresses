...
(function() {
  ...
  // 基本情報の付与
  JSON.parse(Util.simplify(fs.readFileSync(process.argv[2], "utf-8"))).results.bindings.forEach((e: any) => {
  ...
  // 上下関係の接続
  JSON.parse(Util.simplify(fs.readFileSync(process.argv[3], "utf-8"))).results.bindings.forEach((e: any) => {
  ...
  // 前後関係の接続
  JSON.parse(Util.simplify(fs.readFileSync(process.argv[4], "utf-8"))).results.bindings.forEach((e: any) => {
...
readline.createInterface({
  ...
  const col = Util.simplify(line).trim().replace(/"/g, "").split(",");
  ...
  if (parent.children.find((x: any) => x.id === upper) === undefined)
  ...
  parent.children
    .filter((x: any) => x.id === upper)
    .forEach((x: any) => {
      x.max = Math.max(x.max, lower);
    });
  ...
  const dig = function(src: any, container: any) {
    if (!src.visible) {
      src.children.forEach((child: any) => {
        dig(child, container);
      });
      return;
    }
  ...
  // ルートを起点にトラバース
  Object.values(map).filter((e: any) => e.parent.length === 0).forEach(e => {
    dig(e, root);
  });
