...
`normalize()` は住所データの取得に失敗した場合、例外を投げます。取得に失敗した住所を低い `level` で返して、成功したかのように見せることはしません。低い `level` は原則として、入力された住所文字列をそこまでしか判別できなかったことを意味します。

ただし取得できたかどうかの判定は、応答のステータスと本文の長さによるものです。要求した長さと一致する応答は正しいものとして扱うため、配信されているデータ自体が壊れている場合までは検出できません。
