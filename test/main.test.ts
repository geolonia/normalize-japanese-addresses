...
for (const [runtime, normalize] of cases) {
  describe(`tests for ${runtime} entry point`, () => {
    ...
    test('和歌山県東牟婁郡串本町くじ野川一二三四', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "和歌山県", "city": "東牟婁郡串本町", "town": "鬮野川", "addr": "1234", "lat": 33.493026, "lng": 135.784941, "level": 3})
    })
    ...
    test('京都府京都市中京区寺町通御池上る上本能寺前町488番地', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "京都府", "city": "京都市中京区", "town": "上本能寺前町", "addr": "488", "lat": 35.011582, "lng": 135.767914, "level": 3})
    })
    ...
    test('大阪府大阪市中央区大手前２-１', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "大阪府", "city": "大阪市中央区", "town": "大手前二丁目", "addr": "1", "lat": 34.687006, "lng": 135.519317, "level": 3})
    })

    test('北海道札幌市西区二十四軒二条2丁目3番3号', async () => {
      const res = await normalize('北海道札幌市西区24-2-2-3-3')
      expect(res).toBeDeepCloseTo({"pref": "北海道", "city": "札幌市西区", "town": "二十四軒二条二丁目", "addr": "3-3", "lat": 43.074273, "lng": 141.315099, "level": 3})
    })

    test('京都府京都市東山区大和大路2-537-1', async () => {
      const res = await normalize('京都府京都市東山区大和大路2-537-1')
      expect(res).toBeDeepCloseTo({"pref": "京都府", "city": "京都市東山区", "town": "大和大路二丁目", "addr": "537-1", "lat": 34.989944, "lng": 135.770967, "level": 3})
    })
    ...
    test('愛知県蒲郡市旭町17番1号', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "愛知県", "city": "蒲郡市", "town": "旭町", "addr": "17-1", "lat": 34.825785, "lng": 137.218621, "level": 3})
    })

    test('北海道岩見沢市栗沢町万字寿町１−２', async () => {
      const res = await normalize('北海道岩見沢市栗沢町万字寿町１−２')
      expect(res).toBeDeepCloseTo({"pref": "北海道", "city": "岩見沢市", "town": "栗沢町万字寿町", "addr": "1-2", "lat": 43.135248, "lng": 141.986658, "level": 3})
    })
    ...
    test('京都府京都市中京区錦小路通大宮東入七軒町466', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "京都府", "city": "京都市中京区", "town": "七軒町", "addr": "466", "lat": 35.004829, "lng": 135.749797, "level": 3})
    })

    test('栃木県佐野市七軒町2201', async () => {
      const res = await normalize('栃木県佐野市七軒町2201')
      expect(res).toBeDeepCloseTo({"pref": "栃木県", "city": "佐野市", "town": "七軒町", "addr": "2201", "lat": 36.305969, "lng": 139.57389, "level": 3})
    })

    test('京都府京都市東山区大和大路通三条下る東入若松町393', async () => {
      const res = await normalize('京都府京都市東山区大和大路通三条下る東入若松町393')
      expect(res).toBeDeepCloseTo({"pref": "京都府", "city": "京都市東山区", "town": "若松町", "addr": "393", "lat": 35.007967, "lng": 135.774082, "level": 3})
    })

    test('長野県長野市長野東之門町2462', async () => {
      const res = await normalize('長野県長野市長野東之門町2462')
      expect(res).toBeDeepCloseTo({"pref": "長野県", "city": "長野市", "town": "大字長野", "addr": "東之門町2462", "lat": 36.674892, "lng": 138.178449, "level": 3})
    })
    ...
    test('岩手県花巻市下北万丁目１７４−１', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "岩手県", "city": "花巻市", "town": "下北万丁目", "addr": "174-1", "lat": 39.394178, "lng": 141.099889, "level": 3})
    })

    test('岩手県花巻市十二丁目１１９２', async () => {
      const res = await normalize('岩手県花巻市十二丁目１１９２')
      expect(res).toBeDeepCloseTo({"pref": "岩手県", "city": "花巻市", "town": "十二丁目", "addr": "1192", "lat": 39.358268, "lng": 141.122331, "level": 3})
    })

    test('岩手県滝沢市後２６８−５６６', async () => {
      const res = await normalize('岩手県滝沢市後２６８−５６６')
      expect(res).toBeDeepCloseTo({"pref": "岩手県", "city": "滝沢市", "town": "後", "addr": "268-566", "lat": 39.839043, "lng": 141.094179, "level": 3})
    })

    test('青森県五所川原市金木町喜良市千苅６２−８', async () => {
      const res = await normalize('青森県五所川原市金木町喜良市千苅６２−８')
      expect(res).toBeDeepCloseTo({"pref": "青森県", "city": "五所川原市", "town": "金木町喜良市千苅", "addr": "62-8", "lat": 40.904317, "lng": 140.486676, "level": 3})
    })

    test('岩手県盛岡市盛岡駅西通２丁目９番地１号', async () => {
      const res = await normalize('岩手県盛岡市盛岡駅西通２丁目９番地１号')
      expect(res).toBeDeepCloseTo({"pref": "岩手県", "city": "盛岡市", "town": "盛岡駅西通二丁目", "addr": "9-1", "lat": 39.698721, "lng": 141.135252, "level": 3})
    })
    ...
    test('岩手県盛岡市盛岡駅西通２丁目９番地１号', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "岩手県", "city": "盛岡市", "town": "盛岡駅西通二丁目", "addr": "9-1 マリオス10F", "lat": 39.698721, "lng": 141.135252, "level": 3})
    })

    test('東京都文京区千石4丁目15－7', async () => {
      const res = await normalize('東京都文京区千石4丁目15-7')
      expect(res).toBeDeepCloseTo({"pref": "東京都", "city": "文京区", "town": "千石四丁目", "addr": "15-7", "lat": 35.729052, "lng": 139.740683, "level": 3})
    })
    ...
    test('和歌山県東牟婁郡串本町串本 833', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "和歌山県", "city": "東牟婁郡串本町", "town": "串本", "addr": "833", "lat": 33.470358, "lng": 135.779952, "level": 3})
    })
    ...
    test('東京都世田谷区上北沢４の９の２', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "東京都", "city": "世田谷区", "town": "上北沢四丁目", "addr": "9-2", "lat": 35.669726, "lng": 139.620901, "level": 3})
    })

    test('東京都品川区東五反田２丁目５－１１', async () => {
      const res = await normalize('東京都品川区東五反田２丁目５－１１')
      expect(res).toBeDeepCloseTo({"pref": "東京都", "city": "品川区", "town": "東五反田二丁目", "addr": "5-11", "lat": 35.624169, "lng": 139.72819, "level": 3})
    })

    test('東京都世田谷区上北沢四丁目2-1', async () => {
      const res = await normalize('東京都世田谷区上北沢四丁目2-1')
      expect(res).toBeDeepCloseTo({"pref": "東京都", "city": "世田谷区", "town": "上北沢四丁目", "addr": "2-1", "lat": 35.669726, "lng": 139.620901, "level": 3})
    })
    ...
    test('東京都品川区西五反田2丁目31-6', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "東京都", "city": "品川区", "town": "西五反田二丁目", "addr": "31-6", "lat": 35.626368, "lng": 139.721005, "level": 3})
    })
    ...
    test('大阪府大阪市此花区西九条三丁目２－１６', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "大阪府", "city": "大阪市此花区", "town": "西九条三丁目", "addr": "2-16", "lat": 34.684074, "lng": 135.467031, "level": 3})
    })
    ...
    test('千葉県鎌ヶ谷市中佐津間２丁目１５－１４－９', async () => {
      const res = await normalize('千葉県鎌ヶ谷市中佐津間２丁目１５－１４－９')
      expect(res).toBeDeepCloseTo({"pref": "千葉県", "city": "鎌ケ谷市", "town": "中佐津間二丁目", "addr": "15-14-9", "lat": 35.800253, "lng": 140.002133, "level": 3})
    })
    ...
    test('岐阜県不破郡関ケ原町関ヶ原１７０１−６', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "岐阜県", "city": "不破郡関ケ原町", "town": "大字関ケ原", "addr": "1701-6", "lat": 35.368524, "lng": 136.464997, "level": 3})
    })
    ...
    test('東京都町田市木曽東4丁目14-イ22', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "東京都", "city": "町田市", "town": "木曽東四丁目", "addr": "14-イ22", "lat": 35.564817, "lng": 139.429661, "level": 3})
    })
    ...
    test('岩手県花巻市南万丁目127', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "岩手県", "city": "花巻市", "town": "南万丁目", "addr": "127", "lat": 39.387522, "lng": 141.088029, "level": 3})
    })

    test('和歌山県東牟婁郡串本町田並1512', async () => {
      const res = await normalize('和歌山県東牟婁郡串本町田並1512')
      expect(res).toBeDeepCloseTo({"pref": "和歌山県", "city": "東牟婁郡串本町", "town": "田並", "addr": "1512", "lat": 33.48681, "lng": 135.717844, "level": 3})
    })

    test('神奈川県川崎市多摩区東三田1-2-2', async () => {
      const res = await normalize('神奈川県川崎市多摩区東三田1-2-2')
      expect(res).toBeDeepCloseTo({"pref": "神奈川県", "city": "川崎市多摩区", "town": "東三田一丁目", "addr": "2-2", "lat": 35.612653, "lng": 139.549014, "level": 3})
    })
    ...
    test('富山県富山市三番町1番23号', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "富山県", "city": "富山市", "town": "三番町", "addr": "1-23", "lat": 36.688141, "lng": 137.217397, "level": 3})
    })
    ...
    test('富山県富山市中央通り3-1-23', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "富山県", "city": "富山市", "town": "中央通り三丁目", "addr": "1-23", "lat": 36.689604, "lng": 137.222128, "level": 3})
    })

    test('埼玉県南埼玉郡宮代町大字国納３０9－１', async () => {
      const res = await normalize('埼玉県南埼玉郡宮代町大字国納３０9－１')
      expect(res).toBeDeepCloseTo({"pref": "埼玉県", "city": "南埼玉郡宮代町", "town": "大字国納", "addr": "309-1", "lat": 36.038996, "lng": 139.697478, "level": 3})
    })
    ...
    test('大阪府高槻市奈佐原２丁目１－２ メゾンエトワール', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "大阪府", "city": "高槻市", "town": "奈佐原二丁目", "addr": "1-2 メゾンエトワール", "lat": 34.861189, "lng": 135.579573, "level": 3})
    })

    test('埼玉県八潮市大字大瀬１丁目１－１', async () => {
      const res = await normalize('埼玉県八潮市大字大瀬１丁目１－１')
      expect(res).toBeDeepCloseTo({"pref": "埼玉県", "city": "八潮市", "town": "大瀬一丁目", "addr": "1-1", "lat": 35.808825, "lng": 139.84291, "level": 3})
    })

    test('岡山県笠岡市大宜1249－1', async () => {
      const res = await normalize('岡山県笠岡市大宜1249－1')
      expect(res).toBeDeepCloseTo({"pref": "岡山県", "city": "笠岡市", "town": "大宜", "addr": "1249-1", "lat": 34.506729, "lng": 133.473295, "level": 3})
    })
    ...
    test('岡山県岡山市中区さい33-2', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "岡山県", "city": "岡山市中区", "town": "さい", "addr": "33-2", "lat": 34.680505, "lng": 133.948429, "level": 3})
    })
    ...
    test('千葉県松戸市栄町３丁目１６６－５', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "千葉県", "city": "松戸市", "town": "栄町三丁目", "addr": "166-5", "lat": 35.803015, "lng": 139.905619, "level": 3})
    })

    test('東京都新宿区三栄町１７－１６', async () => {
      const res = await normalize('東京都新宿区三栄町１７－１６')
      expect(res).toBeDeepCloseTo({"pref": "東京都", "city": "新宿区", "town": "四谷三栄町", "addr": "17-16", "lat": 35.688757, "lng": 139.725668, "level": 3})
    })
    ...
    test('新潟県新潟市中央区礎町通１ノ町１９６８−１', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "新潟県", "city": "新潟市中央区", "town": "礎町通一ノ町", "addr": "1968-1", "lat": 37.920235, "lng": 139.049572, "level": 3})
    })
    ...
    test('新潟県新潟市中央区上大川前通11番町1881-2', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "新潟県", "city": "新潟市中央区", "town": "上大川前通十一番町", "addr": "1881-2", "lat": 37.927874, "lng": 139.049152, "level": 3})
    })
    ...
    test('埼玉県上尾市壱丁目１１１', async () => {
      ...
      expect(res).toBeDeepCloseTo({"pref": "埼玉県", "city": "上尾市", "town": "大字壱丁目", "addr": "111", "lat": 35.957701, "lng": 139.570578, "level": 3})
    })
