import { describe, run, test } from 'node:test';
...
const runTest = (type: 'esm' | 'umd') => {
  return async () => {
    ...
    await page.goto(`file://${path.join(import.meta.dirname, `browser/index-${type}.html`)}`);
    ...
    if (errorElem.length > 0) {
      ...
      assert.strictEqual(errorElem.length, 0, `#error should not exist, instead we have: ${errorValue}`);
    }
